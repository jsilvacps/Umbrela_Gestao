...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      await supabaseAdmin.from("suporte_solicitacoes").insert([{
        nome, estabelecimento, whatsapp, assunto, mensagem,
        empresa_id: empresa_id ?? null,
        status: "aberto",
      }]);
    } catch (dbErr) {
      console.error("[suporte] DB error:", dbErr);
    }

    // ── 2. Envia email via Resend (opcional — só se a chave estiver configurada) ──
