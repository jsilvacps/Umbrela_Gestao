  ...
  return (
    <main style={{ minHeight: "100vh", background: "#0c121a", fontFamily: "Segoe UI, Arial, sans-serif", color: "#e2e8f0", padding: "28px 24px" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        {msg && <div style={{ background: "#14532d", border: "1px solid #16a34a", borderRadius: 10, padding: "10px 16px", marginBottom: 20, color: "#bbf7d0", fontWeight: 700 }}>{msg}</div>}
        ...
        <div style={{ background: "#161e2b", border: "1px solid #1f2d3d", borderRadius: 16, padding: "22px 24px", marginBottom: 24 }}>
          ...
          {novasChaves.length > 0 && (
            <div style={{ marginTop: 18, background: "#0a1a0f", border: "1px solid #16a34a44", borderRadius: 10, padding: 16 }}>
              <div style={{ color: "#86efac", fontWeight: 700, marginBottom: 10, fontSize: 14 }}>✅ Chaves geradas — copie e envie ao cliente:</div>
              {novasChaves.map((c) => (
              ...
              <button onClick={() => navigator.clipboard.writeText(novasChaves.join("\n"))} style={{ ...btnCinza, marginTop: 8 }}>
                📋 Copiar todas
              </button>
            </div>
          )}
        ...
        <div style={{ background: "#161e2b", border: "1px solid #1f2d3d", borderRadius: 16, padding: "22px 24px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, gap: 12, flexWrap: "wrap" }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <input value={busca} onChange={(e) => setBusca(e.target.value)} placeholder="Buscar chave ou cliente..." style={{ ...inp, width: 230 }} />
                    ...
                    return (
                      <tr key={l.id} style={{ borderBottom: "1px solid #1a2535" }}>
                        <td style={{ padding: "10px 10px" }}>
                          <code style={{ color: "#7dd3fc", fontSize: 13, letterSpacing: 0.5 }}>{l.chave}</code>
                        </td>
                        <td style={{ padding: "10px 10px", color: "#cbd5e1" }}>{l.cliente || <span style={{ color: "#475569" }}>—</span>}</td>
                        <td style={{ padding: "10px 10px" }}>
                          <span style={{ color: statusCor[status], fontWeight: 700 }}>{statusLabel[status]}</span>
                        </td>
